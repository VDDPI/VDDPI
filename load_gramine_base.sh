...
load_image() {
	echo "Loading image from registry.vddpi"
	ssh -T registry.vddpi "docker save $IMAGE_NAME:$IMAGE_LABEL | gzip -c" | gunzip | docker load
}

REMOTE_ID=$(ssh -T registry.vddpi "docker images --no-trunc --quiet $IMAGE_NAME:$IMAGE_LABEL 2>/dev/null")
